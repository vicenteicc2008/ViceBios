...
EFI_STATUS ActivityInitialize(ACTIVITY *this, UINT32 width, UINT32 height) {
    ...
    this->evCount = 0;
    this->evIndex = 255;
    status = gBS->AllocatePool(EfiLoaderData, sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL) * width * height, (VOID **)&this->buffer);
...
EFI_STATUS ActivityRenderArea(ACTIVITY *this, EFI_GRAPHICS_OUTPUT_PROTOCOL *graphProtocol, RECT area) {
...
EFI_STATUS ActivityRender(ACTIVITY *this, EFI_GRAPHICS_OUTPUT_PROTOCOL *graphProtocol) {
    ...
    if (this->countInvalid >= 64) {
        ...
        rect.x = 0;
        rect.y = 0;
        rect.w = (INT32)this->width;
        rect.h = (INT32)this->height;
